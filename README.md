...
CodexQuotaTaskbar v0.1.0 是一个 Windows 11 x64 原生常驻工具。它读取当前 Windows 用户已有的 Codex 登录状态，只向 `chatgpt.com` 的两个固定只读额度路径发起 HTTPS 请求，并在当前主显示器的主任务栏内显示 5 小时和周剩余额度。

任务栏嵌入是唯一界面形态；无法安全识别连续空白区时，程序会提示原因并退出，不会降级为悬浮窗。

## 安装与运行

1. 从 GitHub Releases 下载 `CodexQuotaTaskbar-v0.1.0-win-x64.zip`。
...
v0.1.0 没有安装器、自动更新和代码签名。Windows 可能显示“未知发布者”或 SmartScreen 提示；请只从本仓库 Release 下载并核对 SHA-256。目标电脑需安装 Microsoft Visual C++ 2015–2022 x64 运行库。
...
百分比表示剩余额度。悬停使用 Windows 原生 Tooltip 显示重置倒计时、可用重置机会、账号、套餐和最后成功更新时间。左键无操作；右键可立即刷新、切换单行/双行、开关显示项、修改刷新间隔和颜色、设置开机启动、重新检测任务栏或退出。
...
v0.1.0 已验证基线：Windows 11 x64、100% DPI、单显示器、底部非自动隐藏主任务栏。VS2022 x64 Release、CTest 6/6、任务栏交互探针 21/21、单实例、Explorer 重启恢复和清理退出均已通过。

125% / 150% / 200% DPI、双显示器与主显示器切换、任务栏自动隐藏、睡眠恢复、干净 Windows 用户环境和其他 Windows 11 内部版本尚未完成真实验证，不在当前支持承诺内。
